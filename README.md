...
grounded natural-language answer plus a chart — generated by a Claude
tool-use agent that computes real numbers instead of guessing them.

**Live demo:** _TODO: Streamlit Community Cloud link once deployed_
**Screenshot:** _TODO: add a screenshot of the app in action_
...
```
   ...
   question ───────────▶│  Streamlit UI        │
                        ...
                        │  Agent loop          │───────────────────┐
                        │  (askmycity.agent)   │◀──────────────────┘
                        │  Claude tool-use API │   tool result (JSON)
                        └──────────┬───────────┘
...
- **`src/askmycity/schema.py`** — describes the dataset's columns once, and
  builds the Anthropic tool JSON schemas from that description.
...
- **`src/askmycity/agent.py`** — the tool-use loop: sends the question +
  tool definitions to Claude, executes whichever tool it picks, feeds the
  result (or a validation error, so the model can self-correct) back, and
  returns a final answer plus the full tool-call trace.
...
The app reads `ANTHROPIC_API_KEY` from the environment — never hardcode it.
...
point it at `app/streamlit_app.py`, set the `ANTHROPIC_API_KEY` secret, and
deploy.
